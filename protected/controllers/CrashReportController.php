...
public function actionDeleteAllBeforeVer()
	{
		...
		$criteria = new CDbCriteria();
		$criteria->compare('project_id', $projectId);
		$criteria->addCondition('appversion_id < '. $curProjVer);
		...
		$criteria = new CDbCriteria();
		$criteria->compare('project_id', $projectId);
		$criteria->addCondition('id < '. $curProjVer);
		$version_model = AppVersion::model()->findAll($criteria);
		...
		foreach($version_model as $model)
		{
			// Delete row from database table and remove file attachment
			if(!$model->delete())
			{
				throw new CHttpException(404, 'The specified record doesn\'t exist in the VERSION database or could not be deleted.');
			}
		}
	...
	public function actionPackAllByVer()
	{
		...
		$name = "pack_".date_create()->format('Ymd')."_".$projectId."_".$curProjVer;
		$dirName = Yii::app()->getBasePath()."/data/packedReports/".$name;
		if(!@is_dir($dirName))
		{
			if(False==@mkdir($dirName, 0777, TRUE))
			{
				$error = error_get_last();
				throw new CHttpException(500, 'Couldn\'t make directory: '.$error);
			}
		}
		...
		foreach($models as $model)
		{
			...
			// Delete row from database table and remove file attachment
			if(!@copy($model->getLocalFilePath(),$dirName.'/'.$model->id))
			{
				throw new CHttpException(404, 'The specified record doesn\'t exist in the database or could not be packed.');
			}
		}

		MailQueue::mailSend(Yii::app()->user->loadModel()->email, "no-reply@".Yii::app()->request->serverName, 'Pack '.$name, 'Pack '.$name.' is ready');
	...
	public function actionUploadExternal()
	{
		...
		{	
			...
			if(isset($_POST['description']))
				$report->description = utf8_encode($_POST['description']);
